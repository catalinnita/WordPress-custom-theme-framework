...
add_filter( 'of_sanitize_pattern', 'sanitize_text_field' );
...
function of_recognized_font_faces() {
	...
	// Google fonts - if google fonts plugin is installed
	if (function_exists("get_installed_fonts")) {
		$installed_font = get_installed_fonts();
		$options_font = array_merge($default_font, $installed_font);		
	}
	else {
		$options_font = $default_font;		
	}
	// Custom fonts
	get_wpfw_file(get_stylesheet_directory()."/style/fonts/stylesheet");
	global $custom_fonts;

	if (isset($custom_fonts)) {
		if (count($custom_fonts) > 0) {
			$options_font = array_merge($options_font, $custom_fonts);		
		}
	}
